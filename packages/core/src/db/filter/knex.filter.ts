...
import { pathToColumn } from '../helpers/pathToColumn'

import type { Condition, ConditionBetween, Filter, OperatorType } from 'core/db/filter/filter.schema'
...
function convertConditionToWhere(this: KnexOriginal.QueryBuilder, condition: Condition, logic: 'and' | 'or') {
  Object.entries(condition).forEach(([_name, obj]) => {
    const name = pathToColumn(_name) as any

    Object.entries(obj).forEach(([operators, value]) => {
      switch (operators as OperatorType) {
        case 'eq':
